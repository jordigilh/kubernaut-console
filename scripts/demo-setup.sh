...
# kubernaut-console#62: this script used to `helm install` this repo's own
# (now-removed, redundant) chart into a separate kubernaut-console/dex
# namespace pair. It now applies deploy/kind/oauth2-proxy.yaml directly,
# which is namespace/hostname-pinned to kubernaut-system -- the same
# single-namespace convention the Makefile's `deploy` target, this repo's
# own docs, and kubernaut-demo-scenarios all already use.
CONSOLE_NS="kubernaut-system"
DEX_NS="kubernaut-system"
CONSOLE_CLIENT_ID="kubernaut-apifrontend" # matches deploy/kind/oauth2-proxy.yaml's baked-in oauth2-proxy-secret
...
# Step 2: Deploy Console (oauth2-proxy + nginx) via the Kind manifest
echo ""
echo "Step 2: Deploying Console via deploy/kind/oauth2-proxy.yaml..."
kubectl create namespace "$CONSOLE_NS" 2>/dev/null || true

kubectl apply -f "$PROJECT_DIR/deploy/kind/oauth2-proxy.yaml"
kubectl rollout status deployment/kubernaut-console -n "$CONSOLE_NS" --timeout=120s
...
if [ -d "$PROJECT_DIR/dist" ]; then
  ...
  kubectl cp "$PROJECT_DIR/dist/" "$CONSOLE_NS/$CONSOLE_POD:/usr/share/nginx/html/" -c nginx
else
