...
if check_policy_ok "$SP_CM_NAME" "signalprocessing" "policy.rego"; then
  echo "  $SP_CM_NAME: correct package already present -- skipping"
else
...
if check_policy_ok "$AI_CM_NAME" "aianalysis\\.approval" "approval.rego"; then
  echo "  $AI_CM_NAME: correct package already present -- skipping"
else
